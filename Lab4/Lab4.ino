...
//      MOTOR.setSpeedDir(i, DIRF);
    MOTOR.setSpeedDir1(i, DIRF);
    MOTOR.setSpeedDir2(i-1, DIRF);
      delay(time / steps);
    }//end for
...
//gets sonar distance and adjusts it according to result from phase 3 then displays it on LCD screen
float getSonarDistance(){
    float RangeInCentimeters = ultrasonic.MeasureInCentimeters();
    float coefficient = 0.9994;
    float add = 0.5125;
    float compensatedRange = (coefficient * RangeInCentimeters) + add;
    lcd.clear();
    lcd.print(compensatedRange);//0~400cm
    lcd.print(" cm away");
  Serial.print(compensatedRange);
  Serial.print(" cm away \n");
    return compensatedRange;
}
...
{
float power = 50;
float RangeInCentimeters = getSonarDistance();
...
if(RangeInCentimeters < 50){
   ...
   RangeInCentimeters = getSonarDistance();
...
//ramp(0, power, rampDelay(power));
    MOTOR.setSpeedDir1(power - 1.5, DIRF);
    MOTOR.setSpeedDir2(power, DIRF);
